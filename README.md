...
- `data/kkt12-job.json`: source-shaped KKT12 glossary seed with 61 clean character, title, kinship, place, event, object, drug, and terminology entries. Alternate names are separate cards; display-only age/role notes stay in `note`, and easy generic `ฮ่องเต้`/`ฝ่าบาท` plus `สาวใช้โม่หลาน` are not cards.
