...
import { getPlaybackKind, getTtsText, hasSpeechVoice, scaledTtsRate } from '../src/audio.js';
...
test('detects whether a browser has an actual speech voice', () => {
  assert.equal(hasSpeechVoice({ getVoices: () => [] }), false);
  assert.equal(hasSpeechVoice({ getVoices: () => [{ name: 'Thai voice', lang: 'th-TH' }] }), true);
});
